...
from menuSurface import Pictures_from_mp3
from avarage_color import avrg_clr
...
text = pygame.font.SysFont('rubikbold', 20)
...
load_pic = Pictures_from_mp3(f'{os.getcwd()}\\music', f'{os.getcwd()}\\pictures')
load_pic.save_pictures()
load_pictures = os.listdir(f'{os.getcwd()}\\resize images')
...
plus_button_text = pygame.font.SysFont('arial', 30)
minus_button_text = pygame.font.SysFont('arial', 30)
...
volume_plus_button = pygame.draw.rect(surface_for_buttons, (204, 0, 204), (40, 28, 40, 20), 0, 0, 10, 10)
rect_plus_button = volume_plus_button.move(75,490)

volume_minus_button = pygame.draw.rect(surface_for_buttons, (204,0,204), (40, 50, 40, 20), 0, 0, 0, 0, 10, 10)
rect_minus_button = volume_minus_button.move(75,490)

play = play_button_text.render('►', True, (0,0,0))
pause = pause_button_text.render('||', False, (0,0,0))
stop = stop_button_text.render('■', True, (0,0,0))
plus = plus_button_text.render('+', True, (0,0,0))
minus = minus_button_text.render('-', True, (0,0,0))
...
menu_alpha_tracklist.fill((40,40,40))    # поле для отображения треков (360,408)
...
for x in range(len(track_list.get_tracklist())): #Вывод слоев под треки в зависимости от их количества в папке
    ...
    picture = pygame.image.load(f'resize images\\{load_pictures[x]}')
    track_lists_field.fill((40,40,40))
    track_lists_field.blit(artists, (80,10))
    track_lists_field.blit(picture, (10,0))
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 4:
                ...
                picture_filename = f'resize images\\{list_true[it]}.png'
                ...
                color_avrg = avrg_clr(picture_filename)
                polygon_default_color[0] = int(color_avrg[0])
                polygon_default_color[1] = int(color_avrg[1])
                polygon_default_color[2] = int(color_avrg[2])
        ...
        elif event.type == pygame.MOUSEBUTTONDOWN and rect_plus_button.collidepoint(x, y):
            if event.button == 1:
                if volume < 1.0:
                    volume += 0.2
                    pygame.mixer.music.set_volume(volume)

        elif event.type == pygame.MOUSEBUTTONDOWN and rect_minus_button.collidepoint(x, y):
            if event.button == 1:
                if volume > 0:
                    volume -= 0.2
                    pygame.mixer.music.set_volume(volume)
    ...
    surface_for_buttons.blit(plus, (52,22))
    surface_for_buttons.blit(minus, (55, 40))
